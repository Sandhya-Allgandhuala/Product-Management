...
import { Table, Button, Icon, Menu, Form, Confirm} from 'semantic-ui-react';
import AddeditStore from './AddeditStore.jsx'
...
export default class StoreList extends Component {
            ...
            .then(bodyAsObject => {
                if (bodyAsObject.success) {
                    this.setState(state => {
                        ...
                        const newRecords = [...state.StoreData, bodyAsObject.data];
                        return {
    ...
    update(recordToUpdate, callback) {
        ...
        fetch('/Store/UpdateStoreData/' +
            ...
            .then(bodyAsObject => {
                if (bodyAsObject.success) {
                    let updatedRecords = this.state.StoreData.slice();
    ...
    delete(recordToDelete) {
        fetch('/Store/DeleteStoreData/' + recordToDelete.Id)
            .then(response => response.json())
            .then(bodyAsObject => {
                if (bodyAsObject.success) {
                    const updatedStore = this.state.StoreData.filter(record => record.Id != recordToDelete.Id);
                    // Update the state.
                    this.setState({ StoreData: updatedStore });
                }
                else {
                    this.setState({
                        error: bodyAsObject.message
                    ...
                    <React.Fragment>
                        {this.state.showModal && (
                            <AddeditStore
                                ...
                                header={this.state.record.Id ? "Edit Store" : "Add Store"}
                                save={this.save}
                                cancel={this.cancel}
                            />
                        )}
                        ...
                        <div><Index /></div>
                        <br />
                        <Button className="ui primary button" onClick={this.addNew}>Add Store</Button>
                        <Table striped>
                            <Table.Header>
                                <Table.Row>
                            ...
                            <Table.Body>
                                {this.state.StoreData.map(record => (
...
//React call Store list in main id
const app = document.getElementById('Storemain');
ReactDOM.render(
    <StoreList />, app
);
